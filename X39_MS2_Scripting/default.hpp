...
{ \
	...
	DEBUG_CODE(private["_fnc_scriptName"];);
	DEBUG_CODE(if(isNil "_fnc_scriptName") then {_fnc_scriptName = "NA";};)
	#ifdef ALTERNATIVEDEBUGOUTPUT
		DEBUG_CODE(DEBUG_LOG(str [diag_tickTime COMMA _this COMMA _fnc_scriptName COMMA "%1"]);)
	#else
