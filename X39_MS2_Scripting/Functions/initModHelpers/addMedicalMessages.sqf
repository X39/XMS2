...
{
	[compile format["format[localize 'STR_X39_MS2_Scripting_MedicalMessages_Bleeding_AterialDamage', name (_this select 0), %1]", str (_x select HITZONE_CommonName)],		[1, 0, 0, 1],		compile format["[_this, %1] call X39_MS2_fnc_getAterialDamage", _x select HITZONE_NAME], 1] call X39_MS2_fnc_registerMedicalMessage;
	false
}count X39_MS2_var_Internal_HitZones;
