...
_actionIndex = ["REMOVETOURNIQUET"] call X39_MS2_fnc_getActionById;
if(_actionIndex == -1) exitWith {PRINT_ERROR("Cannot get Native XMS2 action 'REMOVETOURNIQUET'"); false};

[
	localize "STR_X39_MS2_Scripting_QuickActions_dropTourniquetAll_Name",						//Name
	parseText localize "STR_X39_MS2_Scripting_QuickActions_dropTourniquetAll_Description",	//Description
	...
	{//RequiredItemsCode
		private "_count";
		_count = ({[_this select 1, _x select 0] call X39_MS2_fnc_getTournique} count X39_MS2_var_Internal_HitZones);
		-_count
	},
	{//ConditionCode
